...
static char *tcp_state[] = {
    "TCP_ESTABILISHED", "TCP_SYN_SENT",  "TCP_SYN_RECV", "TCP_FIN_WAIT1",
...
static inline int hash(int n) { return n & (HASH_SIZE - 1); }
...
static struct netconn_t *new_netconn(unsigned int inode,
                                     struct netconn_t *src) {
...
static struct netconn_t *validate(unsigned int inode, char *s) {
  ...
  bzero(&t, sizeof(t));
  i = sscanf(s, "%x:%x %x:%x %x", &t.s_addr, &t.s_port, &t.d_addr, &t.d_port,
             &t.state);
  if (i != 5)
    return 0;
...
static void read_tcp_conn(void) {
  ...
  while (fgets(buf, sizeof buf, f)) {
    ...
    for (tmp = buf + i; *tmp == ' ' && tmp > buf; tmp--)
      ;
    while (isdigit(*tmp) && tmp > buf)
      tmp--;
    if (sscanf(++tmp, "%d", &inode) != 1)
      continue;
    if (!(tmp = strchr(buf, ':')))
      continue;
    tmp += 2;
