...
static uint64_t round1024(uint64_t val) { return (val + 512) >> 10; }

static void sys_mem_info_print(char *name, uint64_t val, int pagesize,
                               int isPages) {
...
void sys_mem_info(void) {
  ...
  uint64_t realmem, physmem, v_bufspace, swap_total_sysctl, swap_total = 0,
                                                            swap_used = 0;
  ...
  sys_mem_info_print("MemAvailable  ",
                     v_free + v_inactive + v_laundry + v_bufspace / pagesize,
                     pagesize, 1);
