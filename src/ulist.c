...
// static char *hlp = "\001[F1]Help [F9]Menu [ENT]proc all[t]ree [i]dle/cmd
...
static void u_count(char *name, int p) {
  ...
  for (i = 0; i < sizeof prot_tab / sizeof(struct prot_t); i++) {
    t = &prot_tab[i];
    if (strncmp(t->s, name, strlen(t->s)))
      continue;
    t->nr += p;
  }
}
...
struct user_t *alloc_user(struct utmpx *entry) {
  ...
  if ((ppid = get_ppid(u->pid)) == -1)
    ...
    strncpy(u->parent, get_name(ppid), sizeof u->parent - 1);
  u->line = nusers;
...
static void uprint(struct user_t *u, struct wdgt *w) {
  ...
  n = snprintf(w->mwin->gbuf, w->mwin->gbsize,
               "%-14.14s %-9.9s %-6.6s %-19.19s %s", u->parent, u->name, u->tty,
               u->host, toggle ? count_idle(u->tty) : get_w(u->pid));
...
char *proc_ucount(void) {
  ...
  int other =
      nusers - prot_tab[LOCAL].nr - prot_tab[TELNET].nr - prot_tab[SSH].nr;
  snprintf(buf, sizeof buf - 1,
           "\x1%d users: %d local, %d telnet, %d ssh, %d other\x3", nusers,
           prot_tab[LOCAL].nr, prot_tab[TELNET].nr, prot_tab[SSH].nr, other);
  return buf;
}

/*
 * find existing user by tty
 */

static struct user_t *find_user_by_tty(const char *tty) {
  struct list_head *h;
  struct user_t *u;

  list_for_each(h, &users_l) {
    u = list_entry(h, struct user_t, head);
    if (!strncmp(u->tty, tty, UT_LINESIZE))
      return u;
  }
  return NULL;
}
...
/*
 * Check utmpx for logouts or new logins
 */
static void check_utmpx(struct wdgt *w) {
  struct user_t *u;
  struct list_head *h;
  struct utmpx *entry;
  int changed = 0;
  ...
  while ((entry = getutxent()) != NULL) {
    ...
    if (entry->ut_user[0]) {
#endif
      if (!find_user_by_tty(entry->ut_line)) {
        new_user(entry);
        changed = 1;
      }
      continue;
...
void users_init(void) { read_utmpx(); }
...
static void *umsgh(struct wdgt *w, int type, struct wdgt *s, void *d) {
  ...
  switch (type) {
  case MALL_CRSR_REG:
    w->flags |= WDGT_CRSR_SND;
    break;
  case MALL_CRSR_UNREG:
    w->flags &= ~WDGT_CRSR_SND;
    break;
  case MWANT_CRSR_VAL:
    u = cursor_user(w->crsr);
